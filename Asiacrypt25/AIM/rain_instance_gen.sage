...
if n == 64:
    ...
    F.<z> = GF(2^n, modulus=x^20+x^3+1)
elif n==21:
    F.<z> = GF(2^n, modulus=x^21 + x^2 + 1)
elif n==22:
    F.<z> = GF(2^n, modulus=x^22 + x + 1)
elif n==18:
    F.<z> = GF(2^n, modulus=x^18 + x^3 + 1)
elif n==12:
    F.<z> = GF(2^n, modulus=x^12+x^6+x^4+x+1)
elif n==13:
    F.<z> = GF(2^n, modulus=x^13+x^4+x^3+x+1)   
...
for i in range(0, num_rounds - 1):
    ...
    #print("Linearized polynomial coefficients:\n" + str([coeff for coeff in poly_coefficients]))
    '''
    print("inverse Linearized polynomial coefficients:\n" + str([hex(coeff.integer_representation()) for coeff in inverse_poly_coefficients]))
    print("----------")
